...
Signature-based IDS (Snort, Zeek) cannot detect protocol-compliant covert traffic with no known signature.
Classical ML (SVM, Random Forest) requires protocol-specific manual feature engineering that does not generalize.
Deep learning detectors (LSTM, CNN) overfit to training distributions — LSTM accuracy has been documented dropping from 99.4% to 76.9% under cross-technique evaluation.
No existing system has been evaluated against adversarial anti-forensic modifications.
No existing multi-protocol system provides interpretable detection reasoning for forensic investigation.
...
Binary detection (Normal / Covert)
Technique classification (Header Manipulation / Timing Obfuscation / Flow Blending)
Protocol attribution (DNS / ICMP / TCP / UDP)
...
```
6. Install Npcap
Download and install Npcap with WinPcap API compatibility mode enabled.
---
🚀 Usage
Register/login through the dashboard
Select your network interface and click Start Capture
View live detections, technique classification, and protocol breakdown in real time
Inspect individual flows under Flow Detail Analysis
Export detection history via Export CSV on the Logs page
